document.addEventListener("DOMContentLoaded", function () {
  ...
  function showDropdown(key) {
    ...
    let existingPanel = dropdownMenuContainer.querySelector(
      `.dropdown-panel[data-key="${key}"]`
    );
    if (existingPanel) {
    ...
    panels.forEach((panel) => {
      if (panel.dataset.key !== key) {
        ...
        panel._closeTimer = setTimeout(() => {
          if (
            panel.dataset.closing === "true" &&
            panel.parentNode === dropdownMenuContainer
          ) {
  ...
  function hideDropdown() {
    ...
    if (currentPanel) {
      ...
      panelToHide._closeTimer = setTimeout(() => {
        if (
          panelToHide.dataset.closing === "true" &&
          panelToHide.parentNode === dropdownMenuContainer
        ) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const savedLang =
    localStorage.getItem("language") || navigator.language.slice(0, 2) || "ru";
...
hamburgerBtn.addEventListener("click", () => {
  ...
  if (isMenuOpen && window.matchMedia("(max-width:699px)").matches) {
    document
      .querySelectorAll(".nav-menu .submenu.open, .nav-menu .dropdown.open")
      .forEach((el) => {
