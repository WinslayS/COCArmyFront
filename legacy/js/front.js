  ...
  cards.forEach((card) => {
    ...
    if (screenWidth >= 1100) {
      ...
      function updateTopRow(slices, hoveredIndex) {
        if (hoveredIndex === 0) {
          slices[0].style.clipPath = `polygon(0 0, ${55 + hoverOffset}% 0, ${
            50 + hoverOffset
          }% 100%, 0% 100%)`;
          slices[1].style.clipPath = `polygon(${
            55 + hoverOffset
          }% 0, 100% 0, 100% 100%, ${50 + hoverOffset}% 100%)`;
        } else {
          slices[0].style.clipPath = `polygon(0 0, ${55 - hoverOffset}% 0, ${
            50 - hoverOffset
          }% 100%, 0% 100%)`;
      ...
      function updateBottomRow(slices, hoveredIndex) {
        if (hoveredIndex === 2) {
          slices[2].style.clipPath = `polygon(0 0, ${50 + hoverOffset}% 0, ${
            45 + hoverOffset
          }% 100%, 0% 100%)`;
          slices[3].style.clipPath = `polygon(${
            50 + hoverOffset
          }% 0, 100% 0, 100% 100%, ${45 + hoverOffset}% 100%)`;
        } else {
          slices[2].style.clipPath = `polygon(0 0, ${50 - hoverOffset}% 0, ${
            45 - hoverOffset
          }% 100%, 0% 100%)`;
      ...
      function updateVerticalSlices(hoveredIndex) {
        ...
        for (let i = 0; i < totalSlices; i++) {
          const distance = Math.abs(i - hoveredIndex);
          const factor =
            scaleFactors[Math.min(distance, scaleFactors.length - 1)];
          factors[i] = factor;
          totalFactor += factor;
        }
        slices.forEach((slice, i) => {
